...
import fs from "fs";
import * as Generation from "../lib/generations/generation_pb";
import { ArtifactTypeMap, FinishReasonMap } from "../lib/generations/generation_pb";
import { GenerationServiceClient } from "../lib/generations/generation_pb_service";
    ...
    try {
        ...
        const answers = await new Promise<Generation.Answer[]>(
            (resolve, reject) => {
                ...
                stream.on("data", (data: any) => answers.push(data));
...
export function onGenerationComplete(response: GenerationResponse) {
    ...
    if (response.filteredArtifacts.length > 0) {
        return {
            error: `${response.filteredArtifacts.length} artifact` +
    ...
    try {
        ...
        return response.imageArtifacts[0].getBinary_asB64()
